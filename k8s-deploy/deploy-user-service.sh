...
# 部署服务
echo "3. 部署用户服务..."
kubectl apply -f "$SCRIPT_DIR/k8s-deployment-user-service.yaml" -n "$NAMESPACE"

# 等待部署完成
echo "4. 等待服务就绪..."
kubectl rollout status deployment/lemo-service-recommender-user -n "$NAMESPACE" --timeout=300s
...
echo ""
echo "查看服务状态:"
echo "  kubectl get pods -n $NAMESPACE -l app=lemo-service-recommender-user"
echo ""
echo "查看服务日志:"
echo "  kubectl logs -f -n $NAMESPACE -l app=lemo-service-recommender-user"
echo ""
