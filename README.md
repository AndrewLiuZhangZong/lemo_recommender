...
## 🏗️ v2.0 微服务架构（2亿+用户规模）

系统已完成微服务拆分，支持独立部署、扩缩容和水平扩展，对标字节级超大规模场景。

### 架构层次

| 层级 | 服务 | 职责 | 技术栈 |
|------|------|------|--------|
| **接入层** | BFF (Recommender) | 统一入口、编排、缓存 | FastAPI + gRPC Client |
| **在线服务** | Recall Service | 多路召回（协同过滤、ALS、向量、热门） | gRPC + MongoDB + Redis + Milvus |
| | Ranking Service | 精排（DeepFM、LightGBM） | gRPC + ONNX Runtime |
| | Reranking Service | 重排（多样性、新鲜度、规则） | gRPC |
| | User Service | 用户画像、特征查询 | gRPC + MongoDB + Redis |
| | Item Service | 物品管理、元数据查询 | gRPC + MongoDB + Redis |
| | Behavior Service | 行为采集、实时埋点 | gRPC + Kafka |
| **离线服务** | Model Training Service | 模型训练（DeepFM、Wide&Deep） | Celery + ClickHouse |
| | Feature Engineering Service | 特征工程、特征计算 | Celery + ClickHouse |
| | Vector Generation Service | 向量生成、Embedding计算 | Celery + Milvus |
| **实时服务** | Flink Realtime Service | 实时特征、实时聚合 | Flink + ClickHouseSink |
| | Data Sync Service | 数据同步、缓存更新 | Celery + MongoDB + Redis |
| | Realtime Stream Service | 实时推荐流、实时排序 | Kafka + Redis |

### 服务详情

| 服务 | 说明 | 端口 | 资源配置（生产） | K8s部署文件 |
|------|------|------|----------------|------------|
| **BFF (Recommender)** | 推荐API服务（HTTP+gRPC） | 8080 | 2C4G / 副本3-10 | `k8s-deployment-http-grpc.yaml` |
| **Recall Service** | 召回服务 | 8081 | 4C8G / 副本5-20 | `k8s-deployment-recall-service.yaml` |
| **Ranking Service** | 精排服务 | 8082 | 4C8G / 副本3-15 | `k8s-deployment-ranking-service.yaml` |
| **Reranking Service** | 重排服务 | 8083 | 1C2G / 副本2-5 | `k8s-deployment-reranking-service.yaml` |
| **User Service** | 用户服务 | 8084 | 2C4G / 副本3-10 | `k8s-deployment-user-service.yaml` |
| **Item Service** | 物品服务 | 8085 | 2C4G / 副本3-10 | `k8s-deployment-item-service.yaml` |
| **Behavior Service** | 行为服务 | 8086 | 2C4G / 副本3-10 | `k8s-deployment-behavior-service.yaml` |
| **Model Training** | 模型训练 | 8091 | 8C16G / 副本1-3 | `k8s-deployment-model-training-service.yaml` |
| **Feature Engineering** | 特征工程 | 8092 | 4C8G / 副本2-5 | `k8s-deployment-feature-engineering-service.yaml` |
| **Vector Generation** | 向量生成 | 8093 | 4C8G / 副本2-5 | `k8s-deployment-vector-generation-service.yaml` |
| **Data Sync** | 数据同步 | - | 1C2G / 副本2-5 | `k8s-deployment-data-sync-service.yaml` |
| **Realtime Stream** | 实时推荐流 | - | 2C4G / 副本2-5 | `k8s-deployment-realtime-stream-service.yaml` |
| **Flink Realtime** | Flink实时特征 | - | 4C8G / TM×3 | `flink-deployment-realtime-service.yaml` |
| **Worker** | Celery异步任务 | - | 2C4G / 副本2-5 | `k8s-deployment-worker.yaml` |
| **Beat** | Celery定时任务 | - | 1C2G / 副本1 | `k8s-deployment-beat.yaml` |
| **Consumer** | Kafka消费 | - | 1C2G / 副本2-5 | `k8s-deployment-consumer.yaml` |

### 架构图（v2.0）

```
┌─────────────────────────────────────────────────────────────────────────────┐
│                    Istio Service Mesh (K8s Cluster)                          │
│                                                                               │
│  ┌──────────────────────┐                                                    │
│  │   BFF (Recommender)  │  ◄──── HTTP/gRPC 外部流量入口                      │
│  │   • 统一API入口       │  ◄──── 后台管理系统调用入口 (gRPC)                │
│  │   • 编排召回/排序     │                                                    │
│  │   • L1缓存           │                                                    │
│  └──────────────────────┘                                                    │
│           │  gRPC调用 (Istio熔断/限流/重试)                                   │
│  ┌────────┴──────────────────────────────────────────────────────┐          │
│  │                                                                 │          │
│  ▼ 在线服务层                                                      │          │
│  ┌──────────┐  ┌──────────┐  ┌──────────┐  ┌──────────┐         │          │
│  │ Recall   │  │ Ranking  │  │ Rerank   │  │ User     │         │          │
│  │ 多路召回  │  │ DeepFM   │  │ 多样性   │  │ 画像查询  │         │          │
│  └──────────┘  └──────────┘  └──────────┘  └──────────┘         │          │
│  ┌──────────┐  ┌──────────┐                                      │          │
│  │ Item     │  │ Behavior │                                      │          │
│  │ 物品查询  │  │ 埋点采集  │                                      │          │
...
└───────────────────────────────────────────────────────────────────────────┘
         ...
         │  外部依赖服务     │      │   Kafka Topics   │
         │  • MongoDB        │      │  item-events-*   │
         │  • Redis (3级缓存)│      │  user-behaviors  │
         │  • Milvus (向量)  │      │  realtime-recs   │
         │  • ClickHouse     │      └──────────────────┘
         └──────────────────┘
```
...
- **服务发现**: Istio Service Mesh（生产）/ K8s Service DNS（开发）
- **配置管理**: K8s ConfigMap（环境变量）
- **负载均衡**: Istio Envoy Sidecar（Round Robin + 熔断）
- **监控指标**: Prometheus + Grafana
- **链路追踪**: Istio Distributed Tracing
- **安全加密**: Istio mTLS
...
# 1. 部署Istio Service Mesh（可选，生产环境推荐）
./istio/install-istio.sh

# 2. 部署在线服务（核心）
./deploy-http-grpc-service.sh     # BFF入口
./deploy-recall-service.sh        # 召回服务
./deploy-ranking-service.sh       # 精排服务
./deploy-reranking-service.sh     # 重排服务
./deploy-user-service.sh          # 用户服务
./deploy-item-service.sh          # 物品服务
./deploy-behavior-service.sh      # 行为服务

# 3. 部署后台服务（辅助）
./deploy-worker-service.sh        # 模型训练
./deploy-beat-service.sh          # 定时任务
./deploy-consumer-service.sh      # Kafka消费

# 或者一键部署所有微服务
./deploy-all-microservices.sh
```

#### 方式2: Docker Compose（本地开发）
...
├── data-sync/           # 数据同步服务
│   └── main.py
├── realtime-stream/     # 实时推荐流服务
│   └── main.py
├── flink-realtime/      # Flink实时特征服务
│   └── main.py
├── recommender/         # BFF服务（HTTP + gRPC）
...
v2.0架构对后台管理系统**完全兼容**，无需修改任何代码：

| 集成点 | 说明 | 状态 |
|--------|------|------|
| **gRPC接口** | 后台通过Kratos服务发现调用`lemo-service-recommender` | ✅ 兼容 |
| **配置管理** | 场景、物品、模型、实验等配置接口保持不变 | ✅ 兼容 |
| **数据查询** | MongoDB数据结构保持不变 | ✅ 兼容 |
| **服务发现** | BFF统一入口，内部微服务对后台透明 | ✅ 兼容 |

#### 后台集成架构

```
service_manager_server (Go + Kratos)
        │
        │ gRPC调用 (服务发现)
        ▼
lemo-service-recommender (BFF)
        │
        │ 内部gRPC调用 (Istio路由)
        ▼
[Recall|Ranking|Rerank|User|Item|Behavior...]
```

详细文档：
- 🚀 [v2.0服务重构进度](docs/v2.0服务重构进度.md) - 完整的13个服务实施状态
- 📋 [系统优化计划v2.0](docs/系统优化计划v2.0.md) - 架构设计与资源规划
- 🌐 [Istio服务网格部署指南](docs/Istio服务网格部署指南.md) - Service Mesh配置
